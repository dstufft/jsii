...
export function getCompilerOptions(pkg: PackageMetadata): ts.CompilerOptions {
    return {
        target: ts.ScriptTarget.ES2018,
        lib: [ 'es2016', 'es2017.object', 'es2017.string' ],
        module: ts.ModuleKind.CommonJS,
        declaration: true,
        outDir: pkg.tsconfig && pkg.tsconfig.outDir,
        // Set rootDir if outDir was set, so emitted file paths are completely deterministic.
        rootDir: pkg.tsconfig && pkg.tsconfig.outDir && pkg.root,
        strict: true,
...
export async function saveCompilerOptions(packageDir: string) {
    const options: any = { compilerOptions: getCompilerOptions(await readPackageMetadata(packageDir)) };
